# Inverse kinematic computation with the present method (algn)
...
1. Python data of the list ```AL``` (the partitions) and ```BL``` (the CGS) are saved in ```A.py``` and ```B.py```, respectively.
...
1. For saving the output, set ``OUTPUT`` variable different from 0, then the output will be saved in ```E.dat```; otherwise, the output will not be saved.
    ```
    % asir
    [2077] OUTPUT = 1$    
    ```
...
1. Python data of the list ```E``` is saved in ```E.py```.
